#!/usr/bin/env python3
"""Assemble the multi-asset replication evidence ledger (research-only).

Reads the fetched replication cohort ledger + per-contract bars and each
asset's audited underlying 15m replay, joins every option bar to the identical
underlying bar-open timestamp, evaluates the causal component ledger at each
anchor's decision bar, and records research-only forward outcomes that begin
after that bar. No provider calls; no credential handling.
"""
...
def load_option_bars(bars_path: Path) -> list[dict]:
    return json.loads(bars_path.read_text(encoding="utf-8"))["bars"]
...
def main() -> int:
    ...
    underlying_cache: dict[str, list[dict]] = {}
    underlying_by_t_cache: dict[str, dict] = {}

    def bars_for(symbol: str) -> list[dict]:
        if symbol not in underlying_cache:
            underlying_cache[symbol] = load_underlying_bars(args.replay_dir / f"{symbol}_events_15m.csv.gz")
            underlying_by_t_cache[symbol] = {b["t"]: b for b in underlying_cache[symbol]}
        return underlying_cache[symbol]
    ...
    for row in cohort:
        symbol = row["symbol"]
        ticker = row["ticker"]
        underlying = bars_for(symbol)
        underlying_by_t = underlying_by_t_cache[symbol]
        ...
        if signal_idx is not None:
            try:
                ...
                outcome = {"outcome_status": "UNSCORED_INVALID_PATH", "right_censored": None, "observed_bars": 0}

        base = {
            "anchor_id": row["anchor_id"], "symbol": symbol, "ticker": ticker,
            "contract_type": row["contract_type"], "strike": row["strike"],
            "expiration": row["expiration"], "dte_target": row["dte_target"],
            "moneyness_target": row["moneyness_target"], "partition": row["partition"],
        ...
        for optional_field in ("selection_policy", "moneyness_band", "moneyness_distance_pct", "moneyness_pct", "dte"):
            if optional_field in row:
                base[optional_field] = row[optional_field]
        component_rows.append(dict(
            base, ready=ledger["ready"], matched_bars=ledger["matched_bars"],
        ...
        outcome_rows.append(dict(base, **{
            field: outcome.get(field, 0 if field == "observed_bars" else None)
        ...
        for window_name, underlying_bars_forward in TIME_WINDOWS:
            ...
            if signal_under_idx is not None:
                ...
                if signal_idx is not None:
                    try:
                        window_outcome = forward_long_premium_path_until(option_bars, signal_idx, cutoff_ms)
                        ...
                        window_outcome = {"outcome_status": "UNSCORED_INVALID_PATH", "observed_bars": 0}
            window_rows.append(dict(
                base, window=window_name, underlying_bars_forward=underlying_bars_forward,
                cutoff_time_ms=cutoff_ms, censor_reason=censor_reason, right_censored=right_censored,
                **{field: window_outcome.get(field) for field in OUTCOME_FIELDS
        ...
        for reason in ledger["quality_reasons"]:
            rejections.append({"anchor_id": row["anchor_id"], "symbol": symbol, "ticker": ticker,
    ...
    print(json.dumps({"component_rows": len(component_rows), "outcome_rows": len(outcome_rows),
                      ...
                      "rejection_rows": len(rejections), "coverage_cells": len(coverage_rows)}, indent=2))
    return 0
