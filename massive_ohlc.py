...
class _Limiter:
    ...
    Spaces every request 60/N seconds apart. The free plan enforces the
    5/min ceiling tightly enough that a burst (several requests back-to-back)
    triggers 429s, so the limiter must NOT burst -- the research pipeline uses
    this exact fixed-interval approach for 2-year pulls without 429s.
    """
